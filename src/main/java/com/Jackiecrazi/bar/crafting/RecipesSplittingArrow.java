...
import com.Jackiecrazi.bar.Items.arrows.ISplittingArrow;
...
public class RecipesSplittingArrow implements IRecipe {
    ...
    public ItemStack getCraftingResult(InventoryCrafting inventoryCrafting) {
        ...
        for (int i = 0; i < inventoryCrafting.getSizeInventory(); i++) {
            ...
            if (stack != null) {
                ...
                if ((firstArrowStack == null || stack.getItem() == firstArrowStack.getItem())
                    && item instanceof ISplittingArrow) {
                    ISplittingArrow splittingArrow = (ISplittingArrow) item;
                    int damage = stack.getItemDamage();

                    if (!splittingArrow.isSplittingArrow(damage)) {
                        if (firstArrowStack == null) firstArrowStack = stack;
        ...
        if (firstArrowStack != null) {
            ...
            if (extraArrows == 0
                || (splittingArrow.getItemDamageForArrowCount(arrowStackDamage, 0) == extraArrowsZeroDamage)) {
    ...
    public ItemStack getRecipeOutput() {
        if (dummyOutput == null) dummyOutput = new ItemStack(
            ModItems.DerpArrow,
            1,
            ((ItemQuiverModArrow) ModItems.DerpArrow).getItemDamageForArrowCount(0, 4));
