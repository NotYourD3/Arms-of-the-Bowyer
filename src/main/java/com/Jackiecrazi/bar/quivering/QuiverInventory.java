...
public class QuiverInventory implements IInventory {
    ...
    public QuiverInventory(EntityPlayer p, InventoryPlayer ownerInv, int invIndex) {
        ...
        ItemStack stack = invIndex <= ownerInv.getSizeInventory() ? ownerInv.mainInventory[invIndex]
            : PlayerHandler.getPlayerBaubles(p)
                .getStackInSlot(invIndex - ownerInv.getSizeInventory());
        ...
        if (quiverStack != null) {
            ...
            if (tagCompound != null) {
                ...
                for (int i = 0; i < itemList.tagCount(); i++) {
                    ...
                    if (noUpdate) setInventorySlotContentsNoUpdate(nbtStack.getByte("Slot"), quiverStack);
                    else setInventorySlotContents(nbtStack.getByte("Slot"), quiverStack);
                }
            }
        }
    }
    ...
    public void markDirty() {
        ...
        ItemStack quiverStack = playerInvIndex <= playerInv.getSizeInventory() ? playerInv.mainInventory[playerInvIndex]
            : PlayerHandler.getPlayerBaubles(playerItself)
                .getStackInSlot(playerInvIndex - playerInv.getSizeInventory());

        quiverStack.getTagCompound()
            .setTag("quiver", quiverCompound);
    ...
    public void updateDamage() {
        updateDamage(
            playerInvIndex <= playerInv.getSizeInventory() ? playerInv.mainInventory[playerInvIndex]
                : PlayerHandler.getPlayerBaubles(playerItself)
                    .getStackInSlot(playerInvIndex - playerInv.getSizeInventory()));
    }
