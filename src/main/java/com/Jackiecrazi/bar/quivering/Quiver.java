package com.Jackiecrazi.bar.quivering;
...
import com.Jackiecrazi.bar.helpful.LittleBittah;
...
public class Quiver extends Item implements IBauble {
    ...
    public Quiver() {
        ...
        GameRegistry
            .addRecipe(new ItemStack(this, 1, emptyMetadata), "lsl", " l ", 'l', Items.leather, 's', Items.string);

        int craftingMetadata = setColor(emptyMetadata, 16);

        for (int i = 0; i < 16; i++) {
            GameRegistry.addRecipe(
                new ItemStack(this, 1, craftingMetadata),
                "dqd",
                " d ",
                'q',
                new ItemStack(this, 1, setColor(emptyMetadata, i)),
                'd',
        ...
        for (int i = 0; i < 15; i++) {
            ...
            GameRegistry.addRecipe(
                new ItemStack(this, 1, colorMetadata),
                "dqd",
                " d ",
                'q',
                new ItemStack(this, 1, craftingMetadata),
    ...
    public static void removeArrow(EntityPlayer player, ItemStack quiver, int quiverIndex, int arrowIndex) {
        QuiverInventory inv = new QuiverInventory(player, player.inventory, quiverIndex);
    ...
    public IIcon getIconFromDamageForRenderPass(int metadata, int pass) {
        if (pass == 0) {
            ...
            if (damage >= maxDamage) frame = lastFrame;
            else if (damage <= 0) frame = 0;

            return icons[(int) frame];
        }

        if (pass == passes - 1) return strapIcon;
    ...
    public void onWornTick(ItemStack paramItemStack, EntityLivingBase b) {
        if (b.worldObj.rand.nextInt(200) == 3) {
            if (!b.worldObj.isRemote) b.worldObj
                .playSoundAtEntity(b, "mob.skeleton.say", 5.0F, (float) (b.worldObj.rand.nextFloat() * 0.1 + 0.9));
            double x = b.posX;
            double y = b.posY;
            double z = b.posZ;
            List hi = b.worldObj.getEntitiesWithinAABB(
                EntityLivingBase.class,
                AxisAlignedBB.getBoundingBox(x - 32, y - 1, z - 32, x + 32, y + 3, z + 32));
            if (!hi.isEmpty()) {
    ...
    @Override
    public boolean canEquip(ItemStack paramItemStack, EntityLivingBase paramEntityLivingBase) {
        return true;
    }

    @Override
    public boolean canUnequip(ItemStack paramItemStack, EntityLivingBase paramEntityLivingBase) {
        return true;
    }
