...
import com.Jackiecrazi.bar.entities.ArrowSlot;
...
public class QuiverContainer extends Container {
    ...
    public QuiverContainer(InventoryPlayer playerInv, QuiverInventory quiverInv) {
        ...
        if (heldItem.stackTagCompound == null) heldItem.setTagCompound(new NBTTagCompound());
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 9; j++) {
                ...
                else addSlotToContainer(new SlotReadOnly(playerInv, slotIndex, slotPosX, slotPosY));
            }
        }
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (i != playerInv.currentItem) addSlotToContainer(new Slot(playerInv, i, slotPosX, slotPosY));
            else addSlotToContainer(new SlotReadOnly(playerInv, i, slotPosX, slotPosY));
        }
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotIndex < quiverInventory.size) {
                ...
                while (itemsLeft > 0 && i < quiverInventory.size) {
                    ...
                    if (stackInSlot != null && stackInSlot.stackSize < quiverInventory.stackLimit
                        && stackInSlot.getItem() == stack.getItem()
                        && stackInSlot.getItemDamage() == stack.getItemDamage()) {
                        int amount = quiverInventory.stackLimit - stackInSlot.stackSize;

                        if (amount > itemsLeft) amount = itemsLeft;
                ...
                while (itemsLeft > 0 && i < quiverInventory.size) {
                    ...
                    if (stackInSlot == null) {
                        if (getSlot(i).isItemValid(stack)) {
                            ...
                            if (itemsLeft > quiverInventory.stackLimit) newStack.stackSize = quiverInventory.stackLimit;
                            else newStack.stackSize = itemsLeft;
