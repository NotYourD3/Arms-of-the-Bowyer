...
public class EntityQuiverModArrowNew extends EntityArrow
    implements IThrowableEntity, IProjectile, IEntityAdditionalSpawnData {

    private static final String NoWayWTFIsThat = "CL_00001715";
    public String type;
    /**
    ...
    public EntityQuiverModArrowNew(World p_i1755_1_, EntityLivingBase p_i1755_2_, EntityLivingBase p_i1755_3_,
        ...
        if (block.getMaterial() != Material.air) {
            ...
            if (axisalignedbb != null
                ...
                if (this.type != null && this.type.equals("drillarrow")) {
                    if (this.usefulID <= 0) this.inGround = true;
                    else {
                        ...
                        if (hardness > 0) {
                            ...
                            if (toolHardness < 1) toolHardness = 1;
        ...
        if (this.inGround) {
            ...
            if (movingobjectposition != null) {
                if (movingobjectposition.entityHit != null && this.type != null && !this.type.equals("timedarrow")) {
                                    ...
                                    movingobjectposition.entityHit.addVelocity(
                                        this.motionX * (double) this.knockbackStrength
                                            * 0.6000000238418579D
                                            / (double) f4,
                                        0.1D,
                                        this.motionZ * (double) this.knockbackStrength
                                            * 0.6000000238418579D
                                            / (double) f4);
                                }
                            }
                     ...
                     * if(this.worldObj.getBlock(movingobjectposition.blockX, movingobjectposition.blockY,
                     * movingobjectposition.blockZ)!=null){
    ...
    public void onCollideWithPlayer(EntityPlayer p_70100_1_) {
        if (!this.worldObj.isRemote && this.inGround
            && this.arrowShake <= 0
            && this.type != null
            && !(this.type.equals("timedarrow"))) {
            boolean flag = this.canBePickedUp == 1 || this.canBePickedUp == 2 && p_70100_1_.capabilities.isCreativeMode;
            Item arrow = Items.arrow;
            for (int x = 0; x < ModItems.arrowItemList.size(); x++) {
                if (((ModItems.arrowItemList.get(x))) instanceof ItemQuiverModArrow
                    && ModItems.arrowItemList.get(x) != ModItems.TorchArrow) {
                    if (((ItemQuiverModArrow) ModItems.arrowItemList.get(x)).getName()
                        .equals(this.type)) {
        ...
        if (this.type != null && seme != null) {
            ...
            if (type.equals("potionarrow")) {
                if (uke instanceof EntityLivingBase && this.usefulID != 0) {
                    ((EntityLivingBase) uke)
                        .addPotionEffect(new PotionEffect((int) usefulID, this.getDuration(), this.getPotency()));
                }
            }
    ...
    public void performEffectBlock(String type, Entity seme, MovingObjectPosition mop) {
        if (type != null && seme != null) {
            ...
            if (this.type.equals("splashpotionarrow")) {
                List<EntityLivingBase> hi = this.worldObj.getEntitiesWithinAABB(
                    EntityLivingBase.class,
                    AxisAlignedBB.getBoundingBox(x - 2, y - 1, z - 2, x + 2, y + 3, z + 2));
                if (!hi.isEmpty()) {
                    Iterator i = hi.iterator();
                    while (i.hasNext()) {
                        EntityLivingBase l = (EntityLivingBase) i.next();
                        l.addPotionEffect(new PotionEffect((int) usefulID, getDuration(), getPotency()));
                    }
                }
            }
                ...
                if (!this.worldObj.isRemote) {
                    if (this.shootingEntity != null && this.shootingEntity instanceof EntityPlayerMP) {
                        ...
                        if (entityplayermp.playerNetServerHandler.func_147362_b()
                            .isChannelOpen() && entityplayermp.worldObj == this.worldObj) {
                            ...
                            if (!MinecraftForge.EVENT_BUS.post(event)) { // Don't indent to lower patch size
                                ...
                                ((EntityPlayerMP) this.getThrower())
                                    .setPositionAndUpdate(event.targetX, event.targetY, event.targetZ);
                                this.getThrower().fallDistance = 0.0F;
                                this.getThrower()
                                    .attackEntityFrom(DamageSource.fall, event.attackDamage);
                            }
                        }
                    }
                }
