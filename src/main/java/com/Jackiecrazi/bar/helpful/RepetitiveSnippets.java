...
import com.Jackiecrazi.bar.quivering.QuiverInventory;
...
public class RepetitiveSnippets {
    ...
    public static boolean hasQuiver(EntityPlayer p) {
        ...
        for (int rep = 0; rep < PlayerHandler.getPlayerBaubles(p)
            .getSizeInventory(); rep++) {
            ItemStack i = PlayerHandler.getPlayerBaubles(p)
                .getStackInSlot(rep);
    ...
    public static ItemStack getQuiverStack(EntityPlayer p) {
        ...
        for (int x = 0; x < p.inventory.getSizeInventory(); x++) {
            if (p.inventory.getStackInSlot(x) != null && p.inventory.getStackInSlot(x)
                .getItem() == ModItems.quiver) quiver = p.inventory.getStackInSlot(x);
        }
        for (int rep = 0; rep < PlayerHandler.getPlayerBaubles(p)
            .getSizeInventory(); rep++) {
            ItemStack i = PlayerHandler.getPlayerBaubles(p)
                .getStackInSlot(rep);
            if (i != null && i.getItem() == ModItems.quiver) {
                quiver = PlayerHandler.getPlayerBaubles(p)
                    .getStackInSlot(rep);
            }
        }
        return quiver;
    ...
    public static ArrayList<ItemStack> getQuivers(EntityPlayer p) {
        ...
        for (int x = 0; x < p.inventory.getSizeInventory(); x++) {
            if (p.inventory.getStackInSlot(x) != null) {
                if (p.inventory.getStackInSlot(x)
                    .getItem() == ModItems.quiver) quivers.add(p.inventory.getStackInSlot(x));
            }
        }
        if (BAR.BaublesLoaded) {
            for (int x = 0; x < PlayerHandler.getPlayerBaubles(p).stackList.length; x++) {
                ItemStack bau = PlayerHandler.getPlayerBaubles(p)
                    .getStackInSlot(x);
                if (bau != null && bau.getItem() == ModItems.quiver) {
                    quivers.add(
                        PlayerHandler.getPlayerBaubles(p)
                            .getStackInSlot(x));
                }
            }
        }
    ...
    public static InventorySlots getArrowQuiverSlot(EntityPlayer player) {
        ...
        if (quiverIndex >= 0) {
            ...
            if (quiverIndex <= player.inventory.getSizeInventory()) {
                quiverStack = player.inventory.getStackInSlot(quiverIndex);
                System.out.println(
                    "found quiverStack within " + player.inventory.getSizeInventory() + ", commencing the crunch");
            } else if (Loader.isModLoaded("Baubles")) {
                quiverStack = PlayerHandler.getPlayerBaubles(player)
                    .getStackInSlot(quiverIndex - player.inventory.getSizeInventory());
                System.out.println(
                    "found quiverStack out of " + player.inventory.getSizeInventory()
                        + ", commencing the crunch at Baubles slot "
                        + (quiverIndex - player.inventory.getSizeInventory()));
            }
    ...
    public static InventorySlots getArrowSlot(EntityPlayer player) {
        ...
        if (out == null || out.get("arrow") == null) {
            ...
            if (BAR.BaublesLoaded) {
                baubles = PlayerHandler.getPlayerBaubles(player).stackList;
            }
            ItemStack[] both = ObjectArrays.concat(
    ...
    public static int[] getIndexesToUse(EntityPlayer player) {
        ...
        ItemStack[] inv = ObjectArrays
            .concat(player.inventory.mainInventory, player.inventory.armorInventory, ItemStack.class);
            ...
            for (int quiverI = both.length - 1; quiverI >= 0; quiverI--) {
                ...
                if (quiverStack != null && quiverStack.getItem() == ModItems.quiver) {
                    ...
                    for (int arrowI = quiverInv.size - 1; arrowI >= 0; arrowI--) {
                        ...
                        if (arrowStack != null && arrowStack.getItem() == arrowID
                            && arrowStack.getItemDamage() == arrowDamage) {
                            quiverIndex = quiverI;
                            arrowIndex = arrowI;
                            break;
                        }
    ...
    public static void setWornQuiverType(EntityPlayer playerName, int value) {
        playerName.getEntityData()
            .setInteger("wornQuiverType", value);
    }

    public static int getWornQuiverType(EntityPlayer playerName) {
        return playerName.getEntityData()
            .getInteger("wornQuiverType");
    }

    public void setUsingQuiver(EntityPlayer playerName, int metadata) {
        playerName.getEntityData()
            .setInteger("usingQuiverMetadata", metadata);
    }
}
