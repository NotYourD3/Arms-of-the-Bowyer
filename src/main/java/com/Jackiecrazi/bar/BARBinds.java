...
public class BARBinds {
    ...
    public static final KeyBinding switchArrowForward = new KeyBinding(
        "key.BAR.switchArrowForward",
        Keyboard.KEY_EQUALS,
        "key.categories.gameplay");
    public static final KeyBinding switchArrowBackwards = new KeyBinding(
        "key.BAR.switchArrowBackwards",
        Keyboard.KEY_MINUS,
        "key.categories.gameplay");
    public static final KeyBinding switchQuiverMount = new KeyBinding(
        "key.BAR.switchQuiverMount",
        Keyboard.KEY_BACKSLASH,
        "key.categories.gameplay");
    public static final KeyBinding openAQuiver = new KeyBinding(
        "key.BAR.openAQuiver",
        Keyboard.KEY_BACK,
        "key.categories.gameplay");

    private static final KeyBinding[] bindings = new KeyBinding[] { switchArrowForward, switchArrowBackwards,
        switchQuiverMount, openAQuiver };
    ...
    public void pressKey(KeyInputEvent e) {
        if (switchArrowForward.isPressed()) {
            ...
            int tops = RepetitiveSnippets.getArrowTypesHeld(Minecraft.getMinecraft().thePlayer)
                .size();
            if (value == tops)
