...
public class ArrowNameHelper {
    ...
    public static boolean getUseSplittingPlural() {
        String curLanguage = BAR.proxy.mc.getLanguageManager()
            .getCurrentLanguage()
            .getLanguageCode();

        if (useSplittingPlural == -1 || prevLanguage != curLanguage) {
            prevLanguage = curLanguage;
            useSplittingPlural = StatCollector.translateToLocal("quiverModArrow.splitting.usePlural")
                .equals("true") ? 1 : 0;
        }
    ...
    public static String getName(Item item, String unlocName, boolean multipleArrows) {
        ...
        if (multipleArrows && getUseSplittingPlural()) unlocName += ".plural";

        String out = StatCollector.translateToLocal(unlocName)
            .trim();
    ...
    public static String getName(ItemStack stack) {
        ...
        boolean multipleArrows = stackItem instanceof ISplittingArrow
            && ((ISplittingArrow) stackItem).isSplittingArrow(stack.getItemDamage());
        return getName(stackItem, stackItem.getUnlocalizedName(stack), multipleArrows);
    }

    public static String getLocalizedSplittingArrowPrefix(ISplittingArrow splittingArrow, String arrowName,
        int damage) {
    ...
    public static String getFullName(ItemStack stack) {
        ...
        if (stackItem instanceof ISplittingArrow) name = ArrowNameHelper
            .getLocalizedSplittingArrowPrefix((ISplittingArrow) stack.getItem(), name, stack.getItemDamage());
