package com.Jackiecrazi.bar.lenders;
...
import com.Jackiecrazi.bar.ClientProxy;
import com.Jackiecrazi.bar.Items.ModItems;
import com.Jackiecrazi.bar.client.Tex2DRender3D;
import com.Jackiecrazi.bar.helpful.ColorThing;
import com.Jackiecrazi.bar.helpful.RepetitiveSnippets;
import com.Jackiecrazi.bar.helpful.TextureSizeMap;
import com.Jackiecrazi.bar.helpful.TextureSizeMap.TextureSize;
...
public class QuiverRenderPlayerBase extends RenderPlayerBase {
    ...
    private static final ResourceLocation backQuiverTexture = new ResourceLocation(
        "quivermod:textures/wornquiver/backquiver.png");
    private static final ResourceLocation beltQuiverTexture = new ResourceLocation(
        "quivermod:textures/wornquiver/beltquiver.png");
    ...
    private void renderQuiver(EntityLivingBase entity) {
        if (entity instanceof EntityPlayer) {
            ...
            if (quiver != null) {
                ...
                if (renderQuiverIn3DDistanceSqr == -1) renderQuiverIn3DDistanceSqr = 16;// QuiverModConfigManager.renderQuiverIn3DDistance
                                                                                        // *
                                                                                        // QuiverModConfigManager.renderQuiverIn3DDistance;
                 ...
                 * if (animatedPlayer)
                 * {
                 * ModelPlayer modelBiped =
                 * ((thehippomaster.AnimatedPlayer.client.RenderPlayer)renderPlayer).playerModel;
                ...
                if (wornType != 0) {
                    ...
                    if (p.getCurrentArmor(2) != null) expand = -0.0005F;

                    if (p.getCurrentArmor(1) != null) expand = 0.03125F;
                }
                ...
                if (entity.isInsideOfMaterial(Material.water));// motionY -= 0.02F * partialTick;
                else if (entity.isInsideOfMaterial(Material.lava));// motionY -= 0.02F * partialTick;
                else if (!entity.onGround && !entity.isRiding()
                ...
                if (yTargetRot > 25) yTargetRot = 25;
                else if (yTargetRot < -25) yTargetRot = -25;
                ...
                float diff = MathHelper
                    .clamp_float(targetSpeed - playerValues.quiverRotationSpeed, -maxSpeedChange, maxSpeedChange);
    ...
    @Override
    public void afterRenderModel(EntityLivingBase entity, float limbSwing, float limbYaw, float existedTicksPartial,
        float headYawOffset, float pitch, float scale) {
        renderQuiver(entity);
    }
